...
@interface AeroplaneViewController ()
{
    int index;
}
...
@property(nonatomic,strong) NSMutableArray  *logoArr; //存储综合服务图标
...
-(void)loadLogoData
{
    ...
    [dic setValue:self.serviceCode forKey:@"Shop_type"];
...
- (void)initUI
{
    ...
    titleLab.text = [NSString stringWithFormat:@"   %@",self.titleStr];
...
-(void)btnClick:(UIButton *)sender
{
    if ([self.logoArr[sender.tag][@"Shop_out"] isEqualToString:@"ok"]) {
        ...
        if ([self.logoArr[sender.tag][@"Shop_name"] isEqualToString:@"CIP通道"]) {
            self.hidesBottomBarWhenPushed=YES;
            FlowerServiceViewController * flower = [[FlowerServiceViewController alloc]init];
            flower.titleStr = @"CIP通道" ;
            ...
            self.hidesBottomBarWhenPushed=YES;
            FlowerServiceViewController * flower = [[FlowerServiceViewController alloc]init];
            flower.titleStr = self.logoArr[sender.tag][@"Shop_name"] ; ;
            flower.Shop_id = self.logoArr[sender.tag][@"Shop_id"] ;
            [self.navigationController pushViewController:flower animated:YES];
        }
    }
