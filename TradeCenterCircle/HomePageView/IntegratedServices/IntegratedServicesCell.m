...
@implementation IntegratedServicesCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _title.frame = CGRectMake(0, 20, SCREENWIDTH, 45);
...
-(void)refreshCellWithModel:(NSMutableArray *)model
{
    ...
    if (!_bgView) {
        _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 20 + 45, SCREENWIDTH, 105*(index-1))];
...
-(void)btnClick:(UIButton *)sender
{
    if (self.logoTapVCBlock) {
        self.logoTapVCBlock(sender.tag,self.logoArr[sender.tag][@"Shop_name"],self.logoArr[sender.tag][@"Shop_id"],self.logoArr[sender.tag][@"Shop_out"],self.logoArr[sender.tag][@"Shop_url"]);
    }
}
