...
- (void)setStatus:(NSString *)string {
	...
	self.stringLabel.frame = CGRectMake(0, self.imageView.bottom + 15, CGRectGetWidth(_hudView.bounds), 20); //66
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
	...
	if(error)
		self.imageView.image = [UIImage imageNamed:@"SVProgressHUD.bundle/"];
	else
		self.imageView.image = [UIImage imageNamed:@"SVProgressHUD.bundle/"];
